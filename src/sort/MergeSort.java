...
public class MergeSort {
	...
	private static int[] sort(int[] array, int begin, int end) {
		...
		if (begin==end){
			return array;
		} else if( end-begin == 1 && array[begin] > array[end]) {
		...
		merge(array, begin, end, half);
		return array;
	}

	private static void merge(int[] array, int begin, int end, int half) {
		int index = begin;
		int leftIndex=0;
		int rightIndex = 0;
		...
		while(index <= end ) {
			...
			if(rightIndex==right.length || (leftIndex<left.length && left[leftIndex] <= right[rightIndex])){
				//System.out.println("array["+index+"]="+left[leftIndex]);
				array[index++] = left[leftIndex++];
			} else if(rightIndex<right.length){
				//System.out.println("array["+index+"]="+right[rightIndex]);
